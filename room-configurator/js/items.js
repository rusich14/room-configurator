...
$(document).ready(function () {
  var items = [
    ...
    {
      'name': 'Arkray PocketChem UA PU-4010',
      'image': fullPath('arkray_PocketChem UA PU-4010') + '.jpg',
      'model': fullPath('arkray_PocketChem UA PU-4010') + '.obj',
      'type': '4',
      'code': 'some 123'
    },
  ];

  var furnitureDiv = $('#furniture-wrapper');
  var deviceDiv = $('#device-wrapper');

  for (var i = 0; i < items.length; i++) {
    var item = items[i];
    var props = 'model-name="' + item.name + '" model-url="' + item.model + '" model-type="' + item.type + '" model-code="' + item.code + '"';
    var div = item.type !== '4' ? furnitureDiv : deviceDiv;
...
function fullPath(src) {
  return `models/objects/${src}/${src}`;
}
